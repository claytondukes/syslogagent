...
void logger(int severity, wchar_t *text, ...) {
	...
	wsprintf(outText, text, args);
	AfxMessageBox((LPCTSTR)text, MB_ICONSTOP);
}
...
CNTSyslogCtrlDlg::CNTSyslogCtrlDlg(CWnd* pParent /*=nullptr*/)
	: CDialogEx(IDD_NTSYSLOGCTRL_DIALOG, pParent)
{
...
BEGIN_MESSAGE_MAP(CNTSyslogCtrlDlg, CDialogEx)
	ON_WM_SYSCOMMAND()
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	ON_WM_TIMER()
...
BOOL CNTSyslogCtrlDlg::OnInitDialog() {
	...
	ReadRegKey(&reg_primary, L"0.0.0.0", PRIMARY_SYSLOGD_ENTRY);
	ReadRegKey(&reg_secondary, L"0.0.0.0", BACKUP_SYSLOGD_ENTRY);
	...
	ReadRegKey(&reg_FilterArray, L"", EVENTIDFILTERARRAY);
...
void CNTSyslogCtrlDlg::OnTimer(UINT_PTR nIDEvent) {
	...
	if (nIDEvent == 2) { //user changing ip or port settings
		...
		Port = atoi(CT2A(charPort));
		GetDlgItemText(IDC_PORT2, charPort);
		Port2 = atoi(CT2A(charPort));
...
BOOL CNTSyslogCtrlDlg::QueryServiceStatus()
{
	...
	if ((strcmp(CT2A(InstallStatus), "Uninstall")) != 0) {
...
void CNTSyslogCtrlDlg::CheckRestartService(void) {
	...
	//Restart?
	if (AfxMessageBox(L"Service must be restarted for any new settings to take effect. Restart service now?", MB_YESNO | MB_ICONQUESTION) == IDYES) {
		OnStopService();
...
void CNTSyslogCtrlDlg::OnStartService() {
	...
	{
		AfxMessageBox(L"Error while connecting to the registry!\n\nPlease retry.", MB_ICONSTOP);
		return;
	}
	...
	if ((strcmp(CT2A(szBuffer), "") == 0) || (strcmp(CT2A(szBuffer), "0.0.0.0") == 0)) {
...
void CNTSyslogCtrlDlg::OnBnClickedInstall() {
	...
	_tcscat_s(szModulePathname, L"\"");

	GetDlgItemText(IDC_INSTALL, &(InstallStatus[0]), 32);
	if ((strcmp(CT2A(InstallStatus), "Install")) == 0) {

		CNTService myCNTService(Dapointer, L"SyslogAgent", L"SyslogAgent", (DWORD)0xFF, L"Forwards Event logs to Syslog Server");
	...
	else if ((strcmp(CT2A(InstallStatus), "Uninstall")) == 0) {
...
{
	wchar_t temp[256] = L"", hlpfilepath[512];
	...
	swprintf_s(hlpfilepath, L"%s%s", temp, L"Datagram SyslogAgent manual.pdf");
