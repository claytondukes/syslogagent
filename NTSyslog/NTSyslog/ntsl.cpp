 ...
 *----------------------------------------------------------------------------*/
void ntsl_log_error(wchar_t *format, ...)
{
	va_list     args;
    wchar_t        s[MAX_ERROR_LEN];
	
    va_start(args, format);
    swprintf(s, format, args);
    va_end(args);
 ...
 *----------------------------------------------------------------------------*/
void ntsl_log_info(wchar_t *format, ...)
{
	va_list     args;
    wchar_t        s[MAX_ERROR_LEN];
	
    va_start(args, format);
    wsprintf(s, format, args);
    va_end(args);
 ...
 * Write information message to the event log
 *----------------------------------------------------------------------------*/
void ntsl_log_warning(wchar_t *format, ...)
{
	va_list     args;
	wchar_t        s[MAX_ERROR_LEN];
 ...
 *----------------------------------------------------------------------------*/
void ntsl_log_msg(uint16 etype, wchar_t *format, ...)
{
    va_list     args;
    wchar_t        s[MAX_ERROR_LEN];
	...
	#ifndef _DEBUG
	{
		...
		wchar_t		*strings[2] = { s, NULL };

		ReportEvent(hEvent, etype, 0, 0, NULL, 1, 0, (LPCTSTR*)strings, NULL);
	...
	DEBUGSERVICE(Message, (LPTSTR)L"ntsl error handler activated, code %u.",etype);
...
void ntsl_run(bool forwardEvents,int _EventLogPollInterval){
	...
	DEBUGSERVICE(Message, (LPTSTR)L"Lookup own IP.");
	GetOwnIP();
	...
	DEBUGSERVICE(Message, (LPTSTR)L"Launching application logging thread.");
	...
	DEBUGSERVICE(Message, (LPTSTR)L"Entering main event log loop - init done in this thread.");
	...
	do
	{
		...
		if (forwardEvents) { //The function can be disabled from the gui (and just do application logging)
			DEBUGPARSE(Title, (LPTSTR)L"Scanning event logs...");
			...
			DEBUGPARSE(Title, (LPTSTR)L"Sleeping...\n");
		}
