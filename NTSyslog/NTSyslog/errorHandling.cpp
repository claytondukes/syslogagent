...
long WINAPI top_level_exception_filter(_EXCEPTION_POINTERS *exceptioninfo) {
	...
	logger(Error, (LPTSTR)L"Exception! Top level exception handler called. Writing dump file and rethrowing exception!");
...
void CreateMiniDump( EXCEPTION_POINTERS* pep ) 
{
	...
	HANDLE hFile = CreateFile( T2W((LPTSTR)DebugFlags.DebugDumpFilePath), GENERIC_READ | GENERIC_WRITE, 0, NULL, CREATE_NEW, FILE_ATTRIBUTE_NORMAL, NULL );
	...
	{
		logger(Informational, (LPTSTR)L"Dump file already exists.");
	}
...
{
    logger(Error, (LPTSTR)L"SEH Exception, caught in trans_func by _set_se_translator. writing dump file and rethrowing exception.");
	CreateMiniDump(pExp);
