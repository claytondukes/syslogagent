...
void __cdecl initRegistry(char *SyslogAddress) {
	...
	if (RegConnectRegistry( L"", HKEY_LOCAL_MACHINE, &hKeyRemote) != ERROR_SUCCESS){
		DEBUGSERVICE(4, (LPTSTR)L"Failed to open HKEY_LOCAL_MACHINE in registry - settings not read");
		return;
	}
	...
	if (RegCreateKeyEx( hKeyRemote, NTSYSLOG_SOFTWARE_KEY, 0, REG_NONE, REG_OPTION_NON_VOLATILE,
		...
		DEBUGSERVICE(4, (LPTSTR)L"Failed to open or create SyslogAgent key in registry - settings not read");
	...
	if (SyslogAddress!=NULL) {
		...
		if (RegSetValueEx( hKeySoftware, PRIMARY_SYSLOGD_ENTRY, 0, REG_SZ, (LPBYTE ) (LPCTSTR)( SyslogAddress), dwSize) != ERROR_SUCCESS){
			DEBUGSERVICE(4, (LPTSTR)L"Failed to write SyslogIPAdress key to registry.");
	...
	DEBUGSERVICE(4, (LPTSTR)L"Registry init phase 1 done.");

	hRegTest = NULL;
	if (RegOpenKeyEx(hKeySoftware,APPLICATION_SECTION,0,KEY_WRITE|KEY_READ|WRITE_DAC,&hRegTest)!= ERROR_SUCCESS) {
		initEventLogTypeEntry(hKeySoftware, (wchar_t*)APPLICATION_SECTION, FACILITY_LOCAL7);
	...
	if (RegOpenKeyEx(hKeySoftware,SECURITY_SECTION,0,KEY_WRITE|KEY_READ|WRITE_DAC,&hRegTest)!= ERROR_SUCCESS) {
		initEventLogTypeEntry(hKeySoftware, (wchar_t*)SECURITY_SECTION, FACILITY_SECURITYAUTH);
	...
	if (RegOpenKeyEx(hKeySoftware,SYSTEM_SECTION,0,KEY_WRITE|KEY_READ|WRITE_DAC,&hRegTest)!= ERROR_SUCCESS) {
		initEventLogTypeEntry(hKeySoftware,(wchar_t*)SYSTEM_SECTION, FACILITY_SYSTEM);
		regPermSet.Format(L"HKEY_LOCAL_MACHINE\\%s\\%s",NTSYSLOG_SOFTWARE_KEY,SYSTEM_SECTION);
		AddPermissions(regPermSet);
	} else 
		RegCloseKey(hRegTest);

	DEBUGSERVICE(4, (LPTSTR)L"Registry init phase 2 done.");
	...
	if (RegCreateKeyEx( hKeySoftware, APPLICATIONLOGS, 0, REG_NONE, REG_OPTION_NON_VOLATILE,KEY_WRITE|KEY_READ|WRITE_DAC, NULL, &hRegTest, &dwValue) == ERROR_SUCCESS){
		regPermSet.Format(L"%s\\%s",NTSYSLOG_SOFTWARE_KEY,APPLICATIONLOGS);
		AddPermissions(regPermSet);
	} else
		RegCloseKey(hRegTest);

	DEBUGSERVICE(4, (LPTSTR)L"Registry init phase 3 done.");

	status=AddPermissions(L"SYSTEM\\CurrentControlSet\\Services\\EventLog");
	if (status) {
		DEBUGSERVICE(4, (LPTSTR)L"Failed to write permissions to registry, with error %d",status);
	}
	...
	status = RegOpenKeyEx(HKEY_LOCAL_MACHINE, L"SYSTEM\\CurrentControlSet\\Services\\EventLog", 0, KEY_READ, &hReg);
   	if (status == ERROR_SUCCESS){
		...
		while (RegEnumKey(hReg, i++, (LPTSTR)buffer, sizeof(buffer)) == ERROR_SUCCESS)	
		{
			...
			if (RegOpenKeyEx(hKeySoftware, T2W((LPTSTR)buffer),0,KEY_READ,&hRegTest)!= ERROR_SUCCESS)
				initEventLogTypeEntry(hKeySoftware, T2W((LPTSTR)buffer), FACILITY_SYSTEM);
			else
				RegCloseKey(hRegTest);
		}
		RegCloseKey(hReg);
...
void __cdecl ReadSettings(int *_syslogaPort, int *_syslogBackupPort, bool *forwardEvents,int *EventLogPollInterval) {
	...
	// Connect to the registry on HKLM
	if (RegConnectRegistry( ((LPCTSTR)""), HKEY_LOCAL_MACHINE, &hKeyRemote) != ERROR_SUCCESS){
		DEBUGSERVICE(4, (LPTSTR)L"Failed to open HKEY_LOCAL_MACHINE in registry - settings not read");
		return;
	}
	...
	if (RegCreateKeyEx( hKeyRemote, NTSYSLOG_SOFTWARE_KEY, 0, REG_NONE, REG_OPTION_NON_VOLATILE,
		...
		RegCloseKey( hKeyRemote);
		DEBUGSERVICE(4, (LPTSTR)L"Failed to open SyslogAgent key in registry - settings not read");
		return;
	}
...
void initFilterEventArray( DWORD *A) {
	...
	strcpy_s(theList,CT2A(filterArray.GetBuffer()));
	...
	do {
		...
		if (status==0) {
			*A=-1;
			logger(Error, (LPTSTR)L"EventIDFilterList in registry contained invalid characters. Only numbers, comma and spaces are allowed. Complete filter list possibly not read.");
			break;
		}
		*A=anEventId;
		A++;
...
//	char		ownIP[32];
	...
	// Connect to the registry on HKLM
	if (RegConnectRegistry( ((LPCTSTR)""), HKEY_LOCAL_MACHINE, &hKeyRemote) != ERROR_SUCCESS){
		DEBUGSERVICE(Message, (LPTSTR)L"Failed to open HKEY_LOCAL_MACHINE in registry - settings not read");
		return 0;
	}
	// Create the SOFTWARE\Datagram key or open it if it exists
	if (RegCreateKeyEx( hKeyRemote, NTSYSLOG_SOFTWARE_KEY, 0, REG_NONE, REG_OPTION_NON_VOLATILE,
						KEY_WRITE|KEY_READ, NULL, &hKeySoftware, &dwValue) != ERROR_SUCCESS)
	{
		DEBUGSERVICE(Message, (LPTSTR)L"Failed to open SyslogAgent key in registry - settings not read");
		RegCloseKey( hKeyRemote);
		return 0;
	}
	...
	if ((status!= ERROR_SUCCESS)||(strcmp(CT2A(szBuffer),"")==0)||(strcmp(CT2A(szBuffer),"0.0.0.0")==0))	{
	RegCloseKey (hKeySoftware);
	RegCloseKey( hKeyRemote);
        return 0;
	}
