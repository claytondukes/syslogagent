...
HANDLE SyslogQueueMutex = NULL;

extern wchar_t applHostName[128];
extern wchar_t applHostIP[16];
...
void loggerPrivate(char *text,int severity) {
	...
	strcpy_s(&(tempDate[0]),sizeof(tempDate),CT2A(DaTime.Format(L"%b %d %H:%M:%S")));
	if (tempDate[4] == L'0') // Unix style formatting
		tempDate[4] = L' ';

	WCHAR* pwcsTemplate = new WCHAR[256];
	MultiByteToWideChar(CP_ACP, 0, tempDate, -1, (LPWSTR)pwcsTemplate, 256);

	texten.Format(L"<%d>%s ",FacSev, pwcsTemplate);
	texten+=applHostName;
	texten+=L" SyslogAgent: ";

	wchar_t wcsText[256];
	MultiByteToWideChar(CP_ACP, 0, text, -1, (LPWSTR)wcsText, 256);
	texten+=text;

	wcsncpy_s(data.text,texten,MAXBUFLEN);
	delete pwcsTemplate;
...
void logger(int severity,wchar_t *text,...) {
	...
	if (ErrorCount==4){ //Write that we stop producing errors
		ErrorCount++;
		wcscpy(outText,L"Error reporting halted until network connection restored.");
		loggerPrivate((char*)outText,Notice);
		return;
	}

	ErrorCount++;
	swprintf(outText,text,args);
...
bool ping_syslog_server(CString destHost) {
	...
	ip =(DWORD)inet_addr(CT2A(destHost));
	...
	if (icmphandle !=INVALID_HANDLE_VALUE) {
		...
		logger(Error, (LPTSTR)L"Error. Failed to create icmp handle. Ping cannot be performed. Currently not sending messages. Will retry.");
		Sleep(5000);
		//error. Shuting down?
		return false;
	}
...
static bool GetNetworkAdresses(_threadData *threadData,CString host,CString backupHost) {
	...
	strncpy_s(server_name,sizeof(server_name),CT2A(host.GetBuffer()),255);
	host.ReleaseBuffer();
	strncpy_s(backup_server_name,sizeof(backup_server_name),CT2A(backupHost.GetBuffer()),255);
	...
	if (threadData->server.sin_addr.s_addr == 0) {
		logger(Error, (LPTSTR)L"Primary syslog server IP not identified in fuction GetNetworkAdresses.");
...
static bool initNetworkSettings(_threadData *threadData,int protocol,CString host,int _port,CString backupHost,int _backupport) {
	...
	if (protocol==SOCK_STREAM)
		logger(Informational, (LPTSTR)L"Initiating tcp conn to server.");
	else
		logger(Informational, (LPTSTR)L"Initiating udp conn to server.");
#endif
	...
	if (threadData->sock < 0) {
		logger(Error, (LPTSTR)L"Failed to get socket.");
		WSACleanup();
		return false;
	}
	...
	if (select(0,NULL,&fdStruct,&fdStruct,&tv)==1) { //Connection successful or error. Blocks for up to 3 secs
		if (select(0,NULL,NULL,&fdStruct,&tv_zero)==1) { //error
			...
			logger(Error, (LPTSTR)L"Failed to connect to primary Syslog server. WSA error %d.",status);
		...
		status=WSAGetLastError();
		logger(Error, (LPTSTR)L"Timeout connecting to primary Syslog server. WSA error %d.",status);
		closeTCPconnection(threadData);
		WSACleanup();
		return false;
	}
	...
	DEBUGLOGGER(Message, (LPTSTR)L"Primary conn successful.");
	...
	if (threadData->backupsock < 0)	{
		logger(Error, (LPTSTR)L"Failed to get backup socket.");
		threadData->backupsock=0;
	...
	if (select(0,NULL,&fdStruct,&fdStruct,&tv)==1) { //Connection successful or error. Blocks for up to 3 secs
		if (select(0,NULL,NULL,&fdStruct,&tv_zero)==1) { //error
			status=WSAGetLastError();
			logger(Error, (LPTSTR)L"Failed to connect to mirror Syslog server. WSA error %d.",status);
			if (threadData->backupsock!=0)
				closesocket(threadData->backupsock);
			threadData->backupsock=0;
		...
		status=WSAGetLastError();
		logger(Error, (LPTSTR)L"Timeout connecting to mirror Syslog server. WSA error %d.",status);
		if (threadData->backupsock!=0)
			closesocket(threadData->backupsock);
		threadData->backupsock=0;
		return true;
...
static bool TransmitMessage(_threadData *threadData, bool TCPDelivery, aMess *mess) {
	...
	if (TCPDelivery) {
		...
		switch(LastPos) {
			case 0:
			case 1:
				mess->text[LastPos]=(wchar_t)10;
				mess->text[LastPos+1]=(wchar_t)0;
				LastPos++;
				break;
			case MAXBUFLEN:
				mess->text[MAXBUFLEN-2]=(wchar_t)10;
				mess->text[MAXBUFLEN-1]=(wchar_t)0;
				LastPos=MAXBUFLEN-2;
				break;
			default:
				if (mess->text[LastPos-1]==(wchar_t)10) {
					mess->text[LastPos]=(wchar_t)0;
					break;
				}
				mess->text[LastPos]=(wchar_t)10;
				mess->text[LastPos+1]=(wchar_t)0;
	...
	int size_needed = WideCharToMultiByte(CP_ACP, 0, &mess->text[0], LastPos, 0, 0, 0, 0);
	std::string strNormal(size_needed, 0);
	WideCharToMultiByte(CP_ACP, 0, &mess->text[0], LastPos + 1, &strNormal[0], size_needed, 0, 0);

	int size = MultiByteToWideChar(CP_ACP, MB_COMPOSITE, strNormal.c_str(), strNormal.length(), nullptr, 0);
	std::wstring utf16_str(size, L'\0');
	MultiByteToWideChar(CP_ACP, MB_COMPOSITE, strNormal.c_str(), strNormal.length(), &utf16_str[0], size);
	...
	while(send(threadData->sock, (LPSTR)utf8_str.c_str(), utf8_size, 0) == SOCKET_ERROR) {
		...
		//Error handling
		logger(Error, (LPTSTR)L"Send message failed on socket level.");
		closeTCPconnection(threadData);
	...
	if (threadData->backupsock!=0) {
		while(send(threadData->backupsock, (LPSTR)&(mess->text[0]), 2*LastPos,0) == SOCKET_ERROR) {
			lastError=WSAGetLastError();
			if (lastError==WSAEWOULDBLOCK) { //Buffer full - just wait a while
				Sleep(20);
				continue;
			...
			//Error handling
			logger(Error, (LPTSTR)L"Send message to mirror syslog server failed on socket level.");
			closeTCPconnection(threadData);
			threadData->sock=threadData->backupsock=0;
			Sleep(1000); //Slowdown loop
			break;
...
static bool ReadyToSendMessage(_threadData	*threadData,applSettings *SettingsPtr) {
	...
	if (LastTransmitOK==false) {
		...
		if (WSAStartup(wVersionRequested, &(wsdata)) == SOCKET_ERROR){
			logger(Error, (LPTSTR)L"WSAStartup failed.");
			WSACleanup();
			return false;
		}
	}
...
void outputMain(applSettings *SettingsPtr) {
	...
	DEBUGSERVICE(Message, (LPTSTR)L"Output handling:Thread start.");
	...
	if (WSAStartup(wVersionRequested, &(wsdata)) == SOCKET_ERROR){
		logger(Error, (LPTSTR)L"WSAStartup failed.");
		WSACleanup();
		service_stop_with_error(SOCKET_ERROR, T2W((LPTSTR)"WSAStartup failed - no network"));
		return;
	}
	...
	DEBUGSERVICE(Message, (LPTSTR)L"Output handling:Init done. Starting main output loop.");


	}
	catch(SE_Exception e) {
		logger(Error, (LPTSTR)L"SEH Exception in OutputMain. Event forwarding not functional. Terminating. Error code %u.",e.getSeNumber());
		CreateMiniDump(NULL);
		service_stop_with_error(e.getSeNumber(), (LPTSTR)"SEH Exception in OutputMain. Event forwarding no0t functional. Terminating.");
		AfxEndThread(0,true);
	}

	catch(...) {
		logger(Message, (LPTSTR)L"SEH Exception in OutputMain. Event forwarding not functional. Terminating.");
		CreateMiniDump(NULL);
		service_stop_with_error(-1, (LPTSTR)"Exception in OutputMain. Event forwarding not functional. Terminating.");
		AfxEndThread(0,true);
	}
	...
	while(true) {
		...
		try {
			...
			if (!messQueue.empty()) { //fill if something in queue
				...
				do { //until message successfully sent
					while (!ReadyToSendMessage(&threadData, &loggerInformation)) { //Network failure. Wait for improvement... 
						...
						WSACleanup();
						ReadyToSendAttempts++;
						DEBUGPARSE(Message, (LPTSTR)L"Output handling: Failed to send message. Will slumber and then retry.");
				...
				DEBUGPARSE(Message, (LPTSTR)L"Output handling: Message successfully sent.");
				LastTransmitOK=true;
				ErrorCount=0;
			...
			logger(Error, (LPTSTR)L"SEH Exception in Output loop. Event forwarding not functional. Trying to recover. Error code %u.",e.getSeNumber());
			CreateMiniDump(NULL);
			Sleep(2000);
		}

		catch(...) {
			logger(Message, (LPTSTR)L"SEH Exception in Output loop. Event forwarding not functional. Trying to recover.");
			CreateMiniDump(NULL);
			Sleep(2000);
		}
	}
