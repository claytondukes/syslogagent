...
extern void logger(int severity,wchar_t *text,...);
...
#define addoffset(p,o) ( (byte *) (p) + o )
	...
	/*----------------------------[ private functions ]---------------------------*/
	static wchar_t  *eventlog_lcase(wchar_t *s);
	static int    eventlog_append_data(wchar_t *s, int len, wchar_t* data, BOOL chop);
	static wchar_t **eventlog_strings_to_array(wchar_t *strings, int num);
	...
	static eventlog_data* eventlog_new_log(const wchar_t* name);
	...
	*		failure		-1 
	*----------------------------------------------------------------------------*/
	static int eventlog_append_data(wchar_t *buffer, int n, wchar_t *data, BOOL chop)
	{
		int	rc = -1;

		if ( (buffer != NULL) && (data != NULL) && (*data != NULL)){

			DEBUGPARSE(Message, (LPTSTR)L"Input to event message: %s",data);
	...
	*----------------------------------------------------------------------------*/
	static wchar_t **eventlog_strings_to_array(wchar_t *strings, int num)
	{
		static wchar_t *array[MAX_MSG_STRINGS];
		int			 i;
	...
	*----------------------------------------------------------------------------*/
	static int eventlog_check_event(eventlog_data* eventlog, int id)
	{
		...
		case EVENTLOG_SUCCESS:  //added later by Windows, it seems.
			...
			break;
		default:
			DEBUGPARSE(Warning, (LPTSTR)L"Unknown event log type %d. Event forwarded.",id);
			rc=id;
		}
		return(rc);
	}
	...
	*----------------------------------------------------------------------------*/
	static int eventlog_set_event_type(ntsl_event *event, int id)
	{
		...
		if (NULL != event)
		{
			...
			case EVENTLOG_SUCCESS:			eType = (LPTSTR)NTSL_EVENT_SUCCESS;	     break;  //separate auditsuccess from success?
			case EVENTLOG_ERROR_TYPE:		eType = (LPTSTR)NTSL_EVENT_ERROR;	     break;
			case EVENTLOG_WARNING_TYPE:     eType = (LPTSTR)NTSL_EVENT_WARNING;      break;
			case EVENTLOG_INFORMATION_TYPE: eType = (LPTSTR)NTSL_EVENT_INFORMATION;  break;
			case EVENTLOG_AUDIT_SUCCESS:    eType = (LPTSTR)NTSL_EVENT_SUCCESS;      break;
			case EVENTLOG_AUDIT_FAILURE:    eType = (LPTSTR)NTSL_EVENT_FAILURE;      break;
			};

			if (eType != NULL)
			{
				wcscpy(event->etype,  eType);	
				rc = 0;
			}
		}
	...
	*		failure		-1 
	*----------------------------------------------------------------------------*/
	HMODULE ernoLoadLibrary(wchar_t *singeldll) {
		int i=0;
		DWORD DaError;
		...
		while ((i<MAXLOADEDLIBRARIES)&&(LoadedLibrary[i].name[0]!='\0')) {
			if (wcsstr(singeldll,LoadedLibrary[i].name)!=NULL) {
		...
		//open library
		LoadedLibrary[i].hlib= LoadLibraryEx((LPTSTR)singeldll, NULL, LOAD_LIBRARY_AS_DATAFILE);

		if (LoadedLibrary[i].hlib!=NULL) {
			wcsncpy(LoadedLibrary[i].name,singeldll,sizeof(LoadedLibrary[0].name));
			return LoadedLibrary[i].hlib;
		} else {
	...
	*----------------------------------------------------------------------------*/
	int ScanDLLManuallyForMessage(HINSTANCE hlib, DWORD id, LPTSTR *msg) {
		...
		if ( hgRes == NULL ){
			logger(Warning, (LPTSTR)L"LoadResource failed with error %d.",GetLastError());
			return 0;
		}

		pMRD = (MESSAGE_RESOURCE_DATA *) LockResource( hgRes );
		if ( pMRD == NULL )	{
			logger(Warning, (LPTSTR)L"LockResource failed with error %d.",GetLastError());
			return 0;
		}
	...
	*		failure		-1 
	*----------------------------------------------------------------------------*/
	static int eventlog_set_event_msg(ntsl_event *event, wchar_t *logType,	uint32 id, wchar_t *strings, int numStrings){
		wchar_t	**array; 
		wchar_t	buffer[REG_BUFFER_LEN];
		wchar_t	regSource[REG_BUFFER_LEN];
		wchar_t	dll[REG_BUFFER_LEN];
		...
		wchar_t    singeldll[REG_BUFFER_LEN];
		BOOL    handled = false;
		...
		/* build registry path */
		wsprintf(regSource, L"SYSTEM\\CurrentControlSet\\Services\\EventLog\\%s\\%s", logType,event->source);

		/* load message text */
		if (RegOpenKeyEx(HKEY_LOCAL_MACHINE, (LPTSTR)regSource, 0, KEY_READ,&hkey) == ERROR_SUCCESS){

			if (RegQueryValueEx(hkey, L"EventMessageFile", 0, &regtype, (unsigned char*)buffer, &bufsize) == ERROR_SUCCESS)	{

			} else {
				if (RegQueryValueEx(hkey, L"ProviderGuid", 0, &regtype, (unsigned char*)buffer, &bufsize) == ERROR_SUCCESS)	{
					//Ok, at least we have a providerGuid, which we can use in another part of the registry.
					RegCloseKey(hkey);
					swprintf(regSource, L"SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\WINEVT\\Publishers\\%s",buffer);
					buffer[0] = L'\0';
					if (RegOpenKeyEx(HKEY_LOCAL_MACHINE, (LPTSTR)regSource, 0, KEY_READ,&hkey) == ERROR_SUCCESS){
						RegQueryValueEx(hkey, L"MessageFileName", 0, &regtype, (unsigned char*)buffer, &bufsize);
					}
				}
			}
			RegCloseKey(hkey);
		...
		if (wcslen(buffer)>0) { //success to get a reference to a dll
			if (ExpandEnvironmentStrings((LPTSTR)buffer, (LPTSTR)dll, REG_BUFFER_LEN) > 0){
				...
				while(!eventlog_parse_libs(dll,singeldll,sequencenumber)){
					...
					if ((hlib = ernoLoadLibrary(singeldll)) != NULL){
						...
						if (bytes > 0){
							handled = true;
							event->msg[0] = (char)0;
							eventlog_append_data(event->msg, NTSL_EVENT_LEN, msg, TRUE);
							...
							if ((FormatMessageLastError!=ERROR_MR_MID_NOT_FOUND)&(FormatMessageLastError!=ERROR_RESOURCE_TYPE_NOT_FOUND)) {
								logger(4, (LPTSTR)L"SyslogAgent failed to find error text for a message (FormatMessage error). error: %d dll: %s message: %s, eventid: %d date: %s",FormatMessageLastError,buffer,event->msg,event->id,event->date);
							}
						}
		...
		if (handled){
			...
			for (i=0; i<numStrings; i++){
				if (array[i][0]!=L'\0') {
					tempStr += array[i];
					tempStr += L" ";
//					wcscat(tempBuff, array[i]);
//					wcscat(tempBuff, L" ");
				}
			}

			wchar_t* tempBuff = new wchar_t[tempStr.GetLength()+1];
			memset(tempBuff, L'\0', tempStr.GetLength()+1);
			wsprintf(tempBuff, L"%s", tempStr);
			...
			eventlog_append_data(event->msg, NTSL_EVENT_LEN, (LPTSTR)L"No description available", FALSE);
		}
	...
	*		value	record offset
	*----------------------------------------------------------------------------*/
	DWORD FindFirstReadPos(wchar_t *name,uint32 lastrun) {
		...
		if ((hLog = OpenEventLog(NULL, (LPTSTR)name)) == NULL)
			return 0;
	...
	*----------------------------------------------------------------------------*/
	int ernoReadEventLog(HANDLE hLog,DWORD flags,DWORD offset, DWORD *BytesRead,DWORD *BytesNeeded) {
		...
		if (eventBuffer==NULL) {
			...
			if (eventBuffer==NULL) {
				logger(Error, (LPTSTR)L"Failed to allocate standard buffer of %d bytes of memory.",EVENTLOG_BUFFER_LEN);
				return 0; //Error
			}
		...
		while (true){
			...
			case ERROR_INSUFFICIENT_BUFFER:
				...
				while(EVENTLOG_BUFFER_LEN*multiple<(*BytesNeeded)) {
					...
					if(EVENTLOG_BUFFER_LEN*multiple>3000000) {
						logger(Error, (LPTSTR)L"SyslogAgent rejected suggested needed memory allocation of %d bytes from system to process message. Possible permanent error",*BytesNeeded);
						return 0;
					}
				}
				eventBuffer=(char*)malloc(EVENTLOG_BUFFER_LEN*multiple);
				if (eventBuffer==NULL) {
					logger(Error, (LPTSTR)L"Failed to allocate %d bytes of memory.",EVENTLOG_BUFFER_LEN*multiple);
					return 0; //Error
				}
				record=(EVENTLOGRECORD *)eventBuffer;
			...
			default:
				...
				DEBUGSERVICE(Message, (LPTSTR)L"ReadEventLog failed with error code %d.",lastError);
...
LabelForResettedEventLog:
			...
			if ((hLog = OpenEventLog(NULL, (LPTSTR)eventlog->name)) == NULL) {
				//ntsl_log_error(NTSL_ERROR_EVENT_LOG_ACCESS, eventlog->name);
				DEBUGSERVICE(Message, (LPTSTR)L"Failed to open event log %s. Error: %s", eventlog->name,GetLastError());
				return(-1);
			}

			DEBUGPARSE(Header, (LPTSTR)L"Reading %s event log starting at offset %u.", eventlog->name,eventlog->recordOffset);
			...
			while ((status=ernoReadEventLog(hLog, EVENTLOG_FORWARDS_READ | EVENTLOG_SEEK_READ,recordCounter, &bytes, &next))){
				...
				DEBUGPARSE(Message, (LPTSTR)L"Read %u bytes at position %u with return status %d.",bytes,recordCounter, status);
				...
				if ((ThisIsInitialLoop)&&(eventlog->verificationTime!=record->TimeGenerated)) { //same id as before, but new time! Start from scratch since eventlog has been rotated/deleted
					...
					DEBUGPARSE(Message, (LPTSTR)L"Verification time mismatch! EventLog cleared? starting from the beginning.");
				...
				while (bytes > 0) { 
					...
					//record->EventID &=0x0000FFFF; Dum, dum id? Utan unika, högre delarena blir matchningen mellan id och dll'ens id inte unika--> problem.
					...
					if ( (record->TimeWritten >=  (uint32)*lastrun) && 
						...
						(thisEventIsFilteredOut(record->EventID)==false)) {
							...
							source   = (LPTSTR) ((LPBYTE) record + sizeof(EVENTLOGRECORD));
							computer = source + wcslen(source) + 1;
							strings  = (LPWSTR) ((LPBYTE) record + record->StringOffset);
							...
							if ( (event = (ntsl_event*)malloc0(sizeof(ntsl_event))) == NULL){
								ntsl_log_error((LPTSTR)NTSL_ERROR_EVENT_MALLOC);
								return(-1);
							}	

							event->msg[0] = L'\0';
							//erno2005
							wcscpy(event->facilityName,eventlog->name);
							event->time1970format=record->TimeGenerated;

							wcsftime(event->date, NTSL_DATE_LEN, L"%b %d %H:%M:%S", time);
							if (event->date[4] == L'0') // Unix style formatting
								event->date[4] = L' ';
							
							wcscpy(event->host, eventlog_lcase(computer));
							wcscpy(event->source, eventlog_lcase(source));
							...
							DEBUGPARSE(Message, (LPTSTR)L"Sending parsed record %u to output: %s",record->RecordNumber, event->msg);
			...
			if ((status==0)&&(ThisIsInitialLoop)) { //this only happens if an entry has been removed, i.e. deleted from eventLog. Start from scratch.
				...
				DEBUGPARSE(Message, (LPTSTR)L"Id not found! EventLog cleared? starting from the beginning.");
				eventlog->recordOffset=0;
				ThisIsInitialLoop=false;
				CloseEventLog(hLog); 
				goto LabelForResettedEventLog; //erno only solution to a complete rewrite...(?)
		...
		__except( CreateMiniDump( GetExceptionInformation() ), EXCEPTION_EXECUTE_HANDLER ) {
			logger(Error, (LPTSTR)L"Exception when reading events from %s! Will reattempt reading. Current or previous event text is:%s", eventlog->name, event->msg);
			Sleep(3000);
		}
	...
	*----------------------------------------------------------------------------*/
	static int eventlog_read_lastrun(uint32 *lastrun)
	{
		...
		swprintf_s(buffer, L"SOFTWARE\\%s", DEV_NAME);
		rc = RegOpenKeyEx(HKEY_LOCAL_MACHINE, (LPTSTR)buffer, 0, KEY_READ, &hReg);
		if (rc != ERROR_SUCCESS)
			return(-1);

		rv  = RegQueryValueEx(hReg, (LPTSTR)LAST_RUN_REG, NULL, NULL, (unsigned char*)lastrun, (LPDWORD)&size);
		RegCloseKey(hReg);
	...
	*----------------------------------------------------------------------------*/
	int eventlog_write_lastrun(uint32 *lastrun){
		...
		swprintf(buffer, L"SOFTWARE\\%s", DEV_NAME);
		rc = RegOpenKeyEx(HKEY_LOCAL_MACHINE, (LPTSTR)buffer, 0, KEY_SET_VALUE, &hReg);
		if (rc != ERROR_SUCCESS)
			return(-1);

		rv  = RegSetValueEx(hReg, (LPTSTR)LAST_RUN_REG, 0, REG_DWORD, (unsigned char*)lastrun, size);
		RegCloseKey(hReg);

		if (rv != ERROR_SUCCESS)
			ntsl_die((LPTSTR)NTSL_ERROR_TIME_DATA_WRITE, LAST_RUN_REG);
	...
	*----------------------------------------------------------------------------*/
	static void eventlog_read_reg(eventlog_data* eventlog)
	{
		...
		if (eventlog){
			...
			wsprintf(buffer, L"SOFTWARE\\%s\\%s", DEV_NAME, eventlog->name);
			rc = RegOpenKeyEx(HKEY_LOCAL_MACHINE, (LPTSTR)buffer, 0, KEY_READ, &hReg);
			if (rc == ERROR_SUCCESS)
			{
				// Read "Information" flag from registry.
				rv  = RegQueryValueEx(hReg, L"Information", NULL, NULL, (unsigned char*)&val, (LPDWORD)&size);
				if ( (rv == ERROR_SUCCESS) && (val) )
					eventlog->captureFlags |= EVENTLOG_INFORMATION_FLAG;

				// Read "Information Priority" from registry.
				rv  = RegQueryValueEx(hReg, L"Information Priority", NULL, NULL, (unsigned char*)&val, (LPDWORD)&size);
				if ( (rv == ERROR_SUCCESS) && (val > 0) )
					eventlog->informationPriority = val;

				// Read "Success" flag from registry.
				rv  = RegQueryValueEx(hReg, L"Success", NULL, NULL, (unsigned char*)&val, (LPDWORD)&size);
				if (rv == ERROR_SUCCESS) {
					if (val)
				...
				rv  = RegQueryValueEx(hReg, L"Success Priority", NULL, NULL, (unsigned char*)&val, (LPDWORD)&size);
				if (rv == ERROR_SUCCESS) {
					if (val > 0)
				...
				rv  = RegQueryValueEx(hReg, L"Warning", NULL, NULL, (unsigned char*)&val, (LPDWORD)&size);
				if ( (rv == ERROR_SUCCESS) && (val) )
					eventlog->captureFlags |= EVENTLOG_WARNING_FLAG;

				// Read "Warning Priority" from registry.
				rv  = RegQueryValueEx(hReg, L"Warning Priority", NULL, NULL, (unsigned char*)&val, (LPDWORD)&size);
				if ( (rv == ERROR_SUCCESS) && (val > 0) )
					eventlog->warningPriority = val;

				// Read "Error" flag from registry.
				rv  = RegQueryValueEx(hReg, L"Error", NULL, NULL, (unsigned char*)&val, (LPDWORD)&size);
				if ( (rv == ERROR_SUCCESS) && (val) )
					eventlog->captureFlags |= EVENTLOG_ERROR_FLAG;

				// Read "Error Priority" from registry.
				rv  = RegQueryValueEx(hReg, L"Error Priority", NULL, NULL, (unsigned char*)&val, (LPDWORD)&size);
				if ( (rv == ERROR_SUCCESS) && (val > 0) )
					eventlog->errorPriority = val;

				// Read "Audit Success" flag from registry.
				rv  = RegQueryValueEx(hReg, L"Audit Success", NULL, NULL, (unsigned char*)&val, (LPDWORD)&size);
				if ( (rv == ERROR_SUCCESS) && (val) )
					eventlog->captureFlags |= EVENTLOG_AUDIT_SUCCESS_FLAG;

				// Read "Audit Success Priority" from registry.
				rv  = RegQueryValueEx(hReg, L"Audit Success Priority", NULL, NULL, (unsigned char*)&val, (LPDWORD)&size);
				if ( (rv == ERROR_SUCCESS) && (val > 0) )
					eventlog->auditSuccessPriority = val;

				// Read "Audit Failure" flag from registry.
				rv  = RegQueryValueEx(hReg, L"Audit Failure", NULL, NULL, (unsigned char*)&val, (LPDWORD)&size);
				if ( (rv == ERROR_SUCCESS) && (val) )
					eventlog->captureFlags |= EVENTLOG_AUDIT_FAILURE_FLAG;

				// Read "Audit Failure Priority" from registry.
				rv  = RegQueryValueEx(hReg, L"Audit Failure Priority", NULL, NULL, (unsigned char*)&val, (LPDWORD)&size);
				if ( (rv == ERROR_SUCCESS) && (val > 0) )
			...
			{
				ntsl_die((LPTSTR)NTSL_ERROR_CONFIG_READ, buffer);
			}
		}
	}
	...
	int eventlog_init(){
		...
		rc = RegOpenKeyEx(HKEY_LOCAL_MACHINE, (LPTSTR)buffer, 0, KEY_READ, &hReg);
		if (rc == ERROR_SUCCESS) {
			rv  = RegQueryValueEx(hReg, (LPTSTR)LOOKUPACCOUNTSID, NULL, NULL, (unsigned char*)&LookupAccountSID, (LPDWORD)&size);
			RegCloseKey(hReg);
		}
		...
		rc = RegOpenKeyEx(HKEY_LOCAL_MACHINE, (LPTSTR)buffer, 0, KEY_READ, &hReg);
		if (rc == ERROR_SUCCESS) {
			rv  = RegQueryValueEx(hReg, (LPTSTR)CARRIGERETURNREPLACEMENTCHARINASCII, NULL, NULL, (unsigned char*)&CRReplacementChar, (LPDWORD)&sizeReplacementChar);
			RegCloseKey(hReg);
		}
		...
		rc = RegOpenKeyEx(HKEY_LOCAL_MACHINE, (LPTSTR)buffer, 0, KEY_READ, &hReg);
		if (rc == ERROR_SUCCESS) {
			rv  = RegQueryValueEx(hReg, (LPTSTR)LINEFEEDRETURNREPLACEMENTCHARINASCII, NULL, NULL, (unsigned char*)&LFReplacementChar, (LPDWORD)&sizeReplacementChar);
			RegCloseKey(hReg);
		}
		...
		rc = RegOpenKeyEx(HKEY_LOCAL_MACHINE, (LPTSTR)buffer, 0, KEY_READ, &hReg);
		if (rc == ERROR_SUCCESS) {
			rv  = RegQueryValueEx(hReg, (LPTSTR)TABREPLACEMENTCHARINASCII, NULL, NULL, (unsigned char*)&TABReplacementChar, (LPDWORD)&sizeReplacementChar);
			RegCloseKey(hReg);
		}
		...
		if (eventlog_list != NULL)
		{
			...
			if (rc == ERROR_SUCCESS)
			{
				...
				while (RegEnumKey(hReg, i++, (LPTSTR)buffer, REG_BUFFER_LEN) == ERROR_SUCCESS){
					if (wcsstr(buffer,L"ApplicationLogs")==NULL) {
	...
	*		error		(-1)	
	*----------------------------------------------------------------------------*/
	int eventlog_parse_libs(wchar_t *dllIn, wchar_t *dllOut, uint32 sequence)
	{
		int start = 0;
		int target = 0;
	...
	static int eventlog_set_user(ntsl_event *event, PSID sid, DWORD len){
		...
		while ((i<MAXLOADEDSIDS)&&(LoadedSIDS[i].valid==true)) {
			if (EqualSid(sid,(PSID)(LoadedSIDS[i].SID))!=0) { //hit
				wcsncpy(event->user,LoadedSIDS[i].user,sizeof(LoadedSIDS[i].user));
				wcsncpy(event->domain,LoadedSIDS[i].domain,sizeof(LoadedSIDS[i].domain));
				return 0;		
			}
			i++;
		}
		...
		//Not found
		if (!(LookupAccountSid(NULL, sid, (LPTSTR)LoadedSIDS[i].user, &us, (LPTSTR)LoadedSIDS[i].domain, &ds, &pe)))
			return -1; //fail to lookup

		LoadedSIDS[i].valid=true;
		CopySid(GetLengthSid(LoadedSIDS[0].SID), (PSID)(LoadedSIDS[i].SID), sid);

		wcsncpy(event->user,LoadedSIDS[i].user,wcslen(LoadedSIDS[i].user));
		wcsncpy(event->domain,LoadedSIDS[i].domain,wcslen(LoadedSIDS[i].domain));		
