...
#define _CRT_SECURE_CPP_OVERLOAD_SECURE_NAMES 1
	...
	*--------------------------------------------------------------------------*/
	int WindowsError(int errorCode, char *errorText){
		...
		if(dwLastError >= NERR_BASE && dwLastError <= MAX_NERR) {
			DEBUGPARSE(Message, (LPTSTR)L"Loading netmsg.dll");
		...
		DEBUGPARSE(Message, (LPTSTR)L"Precall FormatMessageA.");
		if(dwBufferLength = FormatMessageA(dwFormatFlags,hModule, dwLastError,MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT),(LPSTR) &MessageBuffer,0,NULL)){
			DEBUGPARSE(Message, (LPTSTR)L"Identified dwBufferLength:%d",dwBufferLength);
			strncpy_s(errorText,250,MessageBuffer,249);
			DEBUGPARSE(Message, (LPTSTR)L"Copy done.");
			//Remove last cr-return
			if (errorText[strlen(errorText)-2]==(char)13) {
				errorText[strlen(errorText)-2]=(char)0;
			}
			DEBUGPARSE(Message, (LPTSTR)L"CRLF Padding cleared.");

			if (dwBufferLength>250) {
				DEBUGPARSE(Message, (LPTSTR)L"Adding dots to long message.");
				strcat_s(errorText,5,"...");
				DEBUGPARSE(Message, (LPTSTR)L"done adding dots to long message.");
			}
			// Free the buffer allocated by the system.
			LocalFree(MessageBuffer);
			DEBUGPARSE(Message, (LPTSTR)L"Free MessageBuffer done.");
			return dwBufferLength;
		} else {
			DEBUGPARSE(Message, (LPTSTR)L"Else: Identified dwBufferLength:%d",dwBufferLength);
		}
	...
	*  with text
	*--------------------------------------------------------------------------*/
	void ParseWindowsErrorCodes(wchar_t *msgString, wchar_t* facilityName) {
		wchar_t *posAfterErrorCode=NULL,*BreakpointInString,*startpos=msgString,*testpos=msgString;
		int errorCode=0,anotherPerCent=0;
		wchar_t tempString[2048]=L"";
		char errorText[256];
		...
		bool UseAuditErrorMessages;
			...
			if (wcscmp(L"Security",facilityName)==0)
...
*/
			while(true) {
				...
				BreakpointInString=wcsstr(testpos,L"%");
				if (BreakpointInString==NULL) break;

				DEBUGPARSE(4, (LPTSTR)L"Found possible error code in message: %s",testpos);
				...
				errorCode= wcstol(BreakpointInString + 1 + anotherPerCent, NULL, 0);
				DEBUGPARSE(Message, (LPTSTR)L"The following error code, if any, was identified: %d",errorCode);
				...
				if (UseAuditErrorMessages) {
					DEBUGPARSE(Message, (LPTSTR)L"Looking up audit error number %d",errorCode);
					if (ernoAuditErrorCodes(errorCode,&(errorText[0]))==0) {
						errorText[0]='\0';
					}
				} else {
					DEBUGPARSE(Message, (LPTSTR)L"Looking up Windows error number %d",errorCode);
					if (WindowsError(errorCode,&(errorText[0]))==0) { //obtained no info
						DEBUGPARSE(Message, (LPTSTR)L"Returned from WindowsError 1.");
						errorText[0]='\0';
					} else {
						DEBUGPARSE(Message, (LPTSTR)L"Returned from WindowsError 2.");
					}
				}

				//Fill tempString with replacement text, starting at the %
				if (errorText[0]!=0) {
					DEBUGPARSE(Message, (LPTSTR)L"Adding errorText to message: %s",errorText);
					if (UseAuditErrorMessages) { //For audit events, hide the entire code
						wsprintf(errorMessage,L" %s",errorText);
					} else {
						wsprintf(errorMessage,L"%%%d (%s)",errorCode,errorText);
					}
				} else {
					DEBUGPARSE(Message, (LPTSTR)L"Adding errorCode only to message: %d",errorCode);
					wsprintf(errorMessage,L"%%%d",errorCode);
				}

				//copy text upto this error code
				wcscat(tempString,startpos);
				wcsncat(tempString,errorMessage,_TRUNCATE);

				startpos=posAfterErrorCode;
				testpos=BreakpointInString+1+anotherPerCent;
				DEBUGPARSE(Message, (LPTSTR)L"Adding one error message done.");
			};

			if (tempString[0]!='\0') {//If anything has been modified, copy new string. Otherwise, leave intact.
				wcscat_s(tempString,posAfterErrorCode);
				wcsncpy_s(msgString,1024,tempString,1023);
			}
	DEBUGPARSE(Message, (LPTSTR)L"Leaving ParseWindowsErrorCodes.");
	}
	...
	*----------------------------------------------------------------------------*/
	int _event_output(ntsl_event *event){
		...
		LFstring[0]=(wchar_t)LFReplacementChar;LFstring[1]=L'\0';
		...
		WCHAR* pwcsHostName = new WCHAR[256];
		MultiByteToWideChar(CP_ACP, 0, charHostname, -1, (LPWSTR)pwcsHostName, 256);

		DEBUGPARSE(Message, (LPTSTR)L"Preparing final string to send.");
		wsprintf(mess.text, L"<%d>%s %s %s%s %d%s%s%s%s%s %s", 
			event->priority, 
			event->date,
			pwcsHostName,
			event->source, 
			...
			event->id, 
			(event->user[0] == 0 ? L"" : L" "),
			(event->domain[0] == 0 ? L"" : event->domain),
			(event->domain[0] == 0 ? L"" : L"\\"),

			(event->user[0] == 0 ? L"" : event->user),
			//raden nedan adderad för att infoga LF, som är vår kod för radbryt om anv finns
			(event->user[0] == 0 ? L"" : LFstring),

			//erno	(event->msg[0] == ':' ? "" : " "), 
			event->msg);
		
		delete[] pwcsHostName;
		DEBUGPARSE(Message, (LPTSTR)L"Inserting message into output queue: %s", mess.text);
