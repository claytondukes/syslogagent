...
#include "assert.h"
//-----LeakWatcher--------------------
#include "../NTSyslog/leakwatcher.h"
#ifdef _DEBUG
...
int GetNextKey(CString *keyName) {
	...
	if (RegEnumKey(hKey, enumerator++, (LPWSTR)buffer, sizeof(wchar_t)*256) == ERROR_SUCCESS) {
		*keyName=buffer;
		return 1;
	}
	*keyName=L"";
...
int GoToRegKey(CString name) {
	...
	if (i != ERROR_SUCCESS) {
		logger(Error, (LPTSTR)L"Failed to go to new sub registry key.");
		hKeyTemp = NULL;
		return -1;
	}
...
int SystemDSNtype() {
	...
	if (RegConnectRegistry(((LPCTSTR)m_csComputer), HKEY_LOCAL_MACHINE, &hKeySoftware) != ERROR_SUCCESS) {
		logger(Error, (LPTSTR)L"Error while connecting to the registry!");
...
int OpenRegistry(CString key) {
	...
	if (RegConnectRegistry(L"", HKEY_LOCAL_MACHINE, &hKeySoftware) != ERROR_SUCCESS) {
		logger(Error, (LPTSTR)L"Error while connecting to the registry (HKEY_LOCAL_MACHINE)!");
		RegCloseKey(hKeySoftware);
		return 0;
	}
	...
	if (i != ERROR_SUCCESS) {
		logger(Error, (LPTSTR)L"Failed to open registry key %s.", csKeyName);
		RegCloseKey(hKeySoftware);
		hKey = NULL;
		hKeySoftware = NULL;
		return 0;
